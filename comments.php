...
<?php if (comments_open()) { ?>
  <section id="respond">
    ...
    <?php } else { ?>
    <!-- Added paragraph tag for label and input -->
      <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
        ...
        <p>
          ...
          <input type="text" class="text span2" name="author" id="author" value="<?php echo esc_attr($comment_author); ?>" tabindex="1" <?php if ($req) echo "aria-required='true'"; ?>>
        </p>
        <p>
          <label for="email"><?php _e('Email', 'roots'); if ($req) _e(' *', 'roots'); ?></label>
          <input type="email" class="text span2" name="email" id="email" value="<?php echo esc_attr($comment_author_email); ?>" tabindex="2" <?php if ($req) echo "aria-required='true'"; ?>>
        </p>
        <p>  
          <label for="url"><?php _e('Website', 'roots'); ?></label>
          <input type="url" class="text span2" name="url" id="url" value="<?php echo esc_attr($comment_author_url); ?>"  tabindex="3">
        <?php } ?>
      </p>
      <p>
      ...
      <p class="clearer"></p>
        <div id="sub-cmt"><input name="submit" class="btn btn-primary" type="submit" id="submit" tabindex="5" value="<?php _e('Submit Comment', 'roots'); ?>"></div>
